...
class ZJUDataset(Dataset):
    def __init__(self,input_dir, b_min, b_max, is_train=True, projection_mode='orthogonal', random_multiview=False, img_size=512, num_views=1, total_samples=1000000, num_sample_points=5000, \
        ...
        self.RENDER_NORMAL = os.path.join(self.root, 'normal_pred1')
    ...
    def get_render(self, subject, num_views, yid=0):
        ...
        if self.is_train:
            random_offset = random.sample(range(-15,14), 3)
        ...
        for vid in view_ids:
            ...
            render = F.interpolate(torch.unsqueeze(render, 0), size=(640, 640), mode='bilinear', align_corners=True)[0]
            normal = F.interpolate(torch.unsqueeze(normal, 0), size=(640, 640), mode='bilinear', align_corners=True)[0]
            mask = F.interpolate(torch.unsqueeze(mask, 0), size=(640, 640), mode='bilinear', align_corners=True)[0]
