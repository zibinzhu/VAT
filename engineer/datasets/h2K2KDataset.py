...
class h2K2KDataset(Dataset):
    def __init__(self,input_dir, b_min, b_max, pipeline=None, is_train=True, projection_mode='orthogonal', random_multiview=False, img_size=512, num_views=1, total_samples=1000000, num_sample_points=5000, \
        num_sample_color=0, sample_sigma=5., check_occ='trimesh', debug=False, span=1, normal=False, fine_pifu = False, crop_windows=512, test=False):
        '''
        2K2K Dataset
        Parameters:
        ...
        Return:
            None
        '''
        super(h2K2KDataset,self).__init__()
        self.is_train = is_train
        self.projection_mode = projection_mode
        self.__name="2K2KDataset"
        ...
        #view from render
        self.root = input_dir
        self.RENDER = os.path.join(self.root, 'PERS', 'RENDER')
        self.RENDER_NORMAL = os.path.join(self.root, 'PERS', 'NORMAL_PRED')
        self.MASK = os.path.join(self.root, 'PERS', 'MASK')
        self.PARAM = os.path.join(self.root, 'PERS', 'PARAM')
        self.OBJ = os.path.join(self.root, 'PLY')
        ...
        self.yaw_list = list(range(0,360,span))
        self.__pitch_list = [0]
        self.normal = normal
    ...
    def select_sampling_method(self, subject):
        ...
        mesh = trimesh.load(os.path.join(self.OBJ, subject, '%s.ply' % subject))
        
        surface_points_1, _ = trimesh.sample.sample_surface(mesh, 48 * self.num_sample_points)
        ...
        sample_points = np.concatenate([sample_points_1, sample_points_2, random_points], 0)
    ...
    def get_render(self, subject, num_views, yid=0):
        ...
        for vid in view_ids:
            ...
            extrinsic_4x4 = np.concatenate([extrinsic, np.array([0., 0., 0., 1.])[None, :]], 0)
            extrinsic = np.linalg.inv(extrinsic_4x4)[:3, :]
            ...
            rays_map, rays_o = self.gen_full_rays(extrinsic, intrinsic, [self.img_size, self.img_size])
            rays_map = rays_map.permute(2, 0, 1)

            mask = torch.FloatTensor((np.array(mask).reshape((self.img_size, self.img_size, -1))))
            mask = mask.reshape(1, self.img_size, self.img_size)
            mask[mask >= 1] = 1.0
            render = self.to_tensor(render) * mask
            normal = self.to_tensor(normal) * mask
            ...
            if self.normal:
                render = torch.cat([render, normal], dim=0)
    ...
    def __getitem__(self,index):
        ...
        res = {
        'name': subject,
        'mesh_path': os.path.join(self.OBJ, subject, subject + '.ply'),
