...
class RPDataset(Dataset):
    def __init__(self,input_dir, b_min, b_max, is_train=True, projection_mode='orthogonal', random_multiview=False, img_size=512, num_views=1, total_samples=1000000, num_sample_points=5000, \
        ...
        self.RENDER_NORMAL = os.path.join(self.root, 'normal_pred')
        ...
        if test:
            ...
            self.yaw = sorted(np.random.choice(range(360), 120))
        print(self.yaw)
    ...
    def get_render(self, subject, num_views, yid=0):
        ...
        if self.is_train:
            random_offset = random.sample(range(-20,20), num_views)
        ...
        for vid in view_ids:
            ...
            mask = torch.sum(torch.FloatTensor((np.array(mask).reshape((self.img_size, self.img_size, -1)))), dim=2) / 255
