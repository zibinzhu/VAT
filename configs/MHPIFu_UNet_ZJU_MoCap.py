...
model = dict(
    PIFu = dict(
        type = 'SRPIFuNet',
        unet = dict(type='HRUNet', in_channels=in_channels, hidden_channels=64),
...
data = dict(
    train = dict(
    type = "RPDataset",
    input_dir = '/data/zhuzibin/Workspace/Dataset/ZJU-MoCap1',
    ...
    type = "RPDataset",
    input_dir = '/data/zhuzibin/Workspace/Dataset/ZJU-MoCap1',
    b_min = np.array([-1.0, -1.2, -1.0]), 
    b_max = np.array([1.0, 0.8, 1.0]),   
    is_train = False,
    random_multiview = False,
    img_size = img_size,
    num_views = num_views,
