...
model = dict(
    PIFu = dict(
        type = 'HRPIFuNet',
        unet = dict(type='UNet', in_channels=in_channels, hidden_channels=64),
        low_attention = dict(type='AttentionDecoder', n_layers=2, q_model=283, k_model=283, v_model=283, d=32, n_head=8, d_inner=256),
        ...
        vis_head = dict(type='PIFuhd_Surface_Head', filter_channels=[475, 256], merge_layer=-1, res_layers=[], norm=None, last_op='None'),
        ...
        fine_head = dict(type='PIFuhd_Surface_Head', filter_channels=[256, 512, 256, 128, 1], merge_layer=-1, res_layers=[1, 2, 3], norm=None, last_op='sigmoid'),
...
data = dict(
    train = dict(
    type = "RPDataset",
    input_dir = '/data/zhuzibin/Workspace/Python/DeepMultiCap-main/dataset/THuman2.0_512',
...
"-------------------------------- config name --------------------------------"
name='THuman2.0_512_normal_pred'
"-------------------------------- render --------------------------------"
render_cfg = dict(
    type='Noraml_Render',
    width = 512,
